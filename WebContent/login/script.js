...
$('button#findid').click(function(event){
   ...
   var inputemail = $('input#input_id_email').val();
   ...
   $.post('findid.jsp', {username: inputname, useremail: inputemail}, function(data){
      if(data.status == 1) setSuccess('아이디 정보가 이메일로 전송되었습니다.');
...
$('button#findpw').click(function(event){
   ...
   var inputemail = $('input#input_pw_email').val();
   
   if(inputid.length == 0 || inputname.length == 0 || inputemail.length == 0){
      setError('정보를 모두 입력해 주십시오.');
      return;
   }
